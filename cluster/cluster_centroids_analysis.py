...
sys.stdout = DualLogger("centroid_analysis_results.txt")
...
for cluster_id in counts.index:
    ...
    # Display top 10 or all significant
    for item in distinctions:
        # Check significance (Loose threshold to capture user request "many features")
        if item['abs_pct'] >= 5.0: # Show anything with >5% deviation
