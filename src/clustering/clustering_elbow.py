...
try:
    df = pd.read_excel(os.path.join(BASE_DIR, 'data', 'raw', 'Data.xlsx'), sheet_name='GenZNCKH1', header=2)
...
plt.ylabel('WCSS (Within-Cluster Sum of Square)', fontsize=12)
plt.xticks(K_range)
plt.grid(True)
# plt.annotate('Elbow Point (k=3)', xy=(3, wcss[2]), xytext=(4, wcss[2]+500), arrowprops=dict(facecolor='red', shrink=0.05))
plt.savefig(os.path.join(BASE_DIR, 'outputs', 'elbow_diagram.png'), dpi=300)
...
# Export Data
output_csv = os.path.join(BASE_DIR, 'data', 'processed', 'clustered_users_final.csv')
